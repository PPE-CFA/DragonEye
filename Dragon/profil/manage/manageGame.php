...
<!--- Navigation --->
  ...
  <?php
      ...
      if(isset($_SESSION['IdType']) AND $_SESSION['IdType'] == "A"){
          ...
          if(isset($_POST['formaddgame'])) {
            ...
            if(!empty($nom) AND !empty($date) AND !empty($price) AND !empty($time) AND !empty($numbersPlayers) AND !empty($idAdmin) AND !empty($idEditeur) AND !empty($age) AND !empty($idCategorie))
            {
                  ...
                  if($nomExist == 0){
                      $insertGame = $bdd->prepare("INSERT INTO deye_jeux(designation,date_sortie,prix,temps_jeux,nb_joueurs,IdPersonne,IdEditeur,IdAge,IdCategorie) VALUES(?,?,?,?,?,?,?,?,?)");
                      $insertGame->execute(array($nom,$date,$price,$time,$numbersPlayers,$idAdmin,$idEditeur,$age,$idCategorie));
    ...
    $allGames = $bdd->query('SELECT IdJeux, designation, date_sortie, prix, temps_jeux, nb_joueurs, deye_personne.nom, deye_editeur.Nom, deye_age.age_requis, deye_categorie.libelle, deye_photo.url 
                              FROM deye_jeux
